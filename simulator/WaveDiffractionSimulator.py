## All to do is find a scaling for the grating and left grid that works at 1920x1080 fullscreen and then compile and embed into website.
...
# In-memory save store: keys will be slot numbers (1, 2, or 3)
SAVE_SLOTS = {}

def get_asset_path(filename):
    """ Get absolute path to resource in assets/ directory, works for dev and PyInstaller """
    # Handle PyInstaller temporary folder (_MEIPASS) or default to script directory
...
def mainMenuOpen(debounce):
    if debounce == False:
        ...
        waveDiffractionIcon = PhotoImage(file=get_asset_path('programIcon.png'))
        try:
            mainMenu.wm_attributes('-transparentcolor', '#ab23ff')
        except Exception:
            pass  # Linux/X11 ignores transparent color keys
    ...
    # Helper function to check and render slots based on the in-memory dictionary
    def render_slot(slot_num, rely_pos, menu_index):
        if slot_num in SAVE_SLOTS:
            fileNamePreviousFile = SAVE_SLOTS[slot_num]["fileName"]
            
            button = Radiobutton(mainMenu, text=fileNamePreviousFile, justify="left", variable=saveFileNumber, value=slot_num)
            button.config(font=('Arial', 25, 'bold'),
                          relief='raised',
                          border=5,
                          pady=45,
                          indicatoron=0)
            button.place(relx=0.5, rely=rely_pos, relwidth=0.3, anchor='n')
            savedSetupsMenu[menu_index] = 1
            return button
        else:
            label = Label(mainMenu,
                          text="Empty",
                          justify="center",
                          bg='#c2c2c2',
                          border=5,
                          ...
                          pady=55,
                          font=('Arial', 22, 'bold'))
            label.place(relx=0.5, rely=rely_pos, anchor='n')
            return label

    # Render Slot 1, 2, and 3 dynamically
    global savedSetup1Button, savedSetup2Button, savedSetup3Button
    savedSetup1Button = render_slot(1, 0.35, 0)
    savedSetup2Button = render_slot(2, 0.53, 1)
    savedSetup3Button = render_slot(3, 0.71, 2)


    

    leftMenuImageUnscaled = PhotoImage(file=get_asset_path('leftMenuImage.png'))
    ...
    rightMenuImageUnscaled = PhotoImage(file=get_asset_path('rightMenuImage.png'))
...
def loadPreviousSystem(saveFileNo, savedSetupsMenuArray): #subroutine to access and validate the values of a save file to be loaded, before calling generateSystem to load
    ...
    slot_num = int(saveFileNo)
    if slot_num in SAVE_SLOTS:
        # Retrieve the uncorrupted list of values directly from memory
        loadedSystemValues = list(SAVE_SLOTS[slot_num]["rawValues"])
    else:
        loadedSystemValues = []
    ...
    # Ruler Position Validation (Directly handles lists [x, y] or string "Default")
    if loadedSystemValues[4] == "Default":
        loadedSystemValues[4] = ["initialRulerXCord", "initialRulerYCord"]
    elif isinstance(loadedSystemValues[4], (list, tuple)):
    ...
    # Protractor Position Validation (Directly handles lists [x, y] or string "Default")
    if loadedSystemValues[5] == "Default":
        loadedSystemValues[5] = ["initialProtractorXCord", "initialProtractorYCord"]
    elif isinstance(loadedSystemValues[5], (list, tuple)):
...
def generateSystem(systemValues):
    ...
    if chosenDiffractionType.get() ==  "Single-Slit":
        ...
        lowerGrating = SimulationApparatus(0.3, 0.425, "singleSlitGratingObjectIcon.png") #creates two instances of simuationApparatus for each part of the single-slit
                                                                                          #grating
        ...
        middleGrating = SimulationApparatus(0.3, 0.388, "middleDoubleSlitGratingIcon.png")
        lowerGrating = SimulationApparatus(0.35, 0.459, "lowerUpperDoubleSlitGratingIcon.png") #creates three instances of simuationApparatus for each part of the
                                                                                                #single-slit grating
    else:
        ...
        gratingObjectsList = []
        for x in range(0,11):
            currentGratingObject = SimulationApparatus(0.3, 0.0037 + (x*0.0758), "diffractionGratingObjectIcon.png")
    ...
    leftScreenGratingArrowIcon = PhotoImage(file=get_asset_path("screenGratingDistanceArrowIconLeft.png"))
    ...
    rightScreenGratingArrowIcon = PhotoImage(file=get_asset_path("screenGratingDistanceArrowIconRight.png"))
    ...
    laserImageIconUnscaled = PhotoImage(file=get_asset_path("laserIcon.png"))
    ...
    laserImageLabel.place(relx=0.02, rely = 0.385, anchor='nw') #places this image in the correct position
    ...
    laserStateONIconUnscaled = PhotoImage(file=get_asset_path("ONSwitchIcon.png"))
    ...
    laserStateOFFIconUnscaled = PhotoImage(file=get_asset_path("OFFSwitchIcon.png"))
    ...
    if laserState.get() == True:
        laserStateONIconLabel.place(relx=0.0335, rely = 0.4025, anchor='nw') #places the image for the switched on laser if it is enabled
    else:
        laserStateOFFIconLabel.place(relx=0.0335, rely = 0.4025, anchor='nw') #places the image for the switched off laser if it is disabled
    ...
    metreScaleArrowIcon = PhotoImage(file=get_asset_path("metreScaleArrowIcon.png"))
...
# Set default / normalized coordinates for Ruler & Protractor
    ...
    global ruler
    global protractor
    ruler = MeasuringEquipment(newRulerXCord, newRulerYCord, "rulerIcon.png", int(systemValues[6]), 4, 50)
    protractor = MeasuringEquipment(newProtractorXCord, newProtractorYCord, "protractorIcon.png", int(systemValues[7]), 3, 110)

    # Ensure items are visible above full-screen canvas layers on Linux/Docker X11
    try:
        ruler.show()
        protractor.show()
...
def saveSetupPrompt():
    ...
    saveSystemFrame.place(relheight=0.55, relwidth=0.4, relx=0.5, rely=0.45, anchor="center") 
    ...
    # Helper function to configure button label based on SAVE_SLOTS dictionary
    def setup_save_button(button, slot_num):
        if slot_num in SAVE_SLOTS:
            fileNameFromPreviousFile = SAVE_SLOTS[slot_num]["fileName"]
            button.config(text=fileNameFromPreviousFile)
        else:
            button.config(text="Empty Save File",
                          justify="center",
                          bg='#c2c2c2',
                          border=5,
                          relief='groove',
    ...
    saveToSetup1Button.config(font=('Arial', 25, 'bold'), relief='raised', border=5, indicatoron=0)
    setup_save_button(saveToSetup1Button, 1)
    ...
    saveToSetup2Button.config(font=('Arial', 25, 'bold'), relief='raised', border=5, indicatoron=0)
    setup_save_button(saveToSetup2Button, 2)
    ...
    saveToSetup3Button.config(font=('Arial', 25, 'bold'), relief='raised', border=5, indicatoron=0)
    setup_save_button(saveToSetup3Button, 3)
...
def saveSystem(): #procedure to save the current setup in the correct file
    ...
    slot_num = int(saveToFileNumber.get())
    
    # Store exact raw values directly into the global dictionary
    SAVE_SLOTS[slot_num] = {
        "fileName": fileName.get(),
        "rawValues": saveFileValues
    }
...
def toggleLaserState():
    if (laserState.get()) == True:
        ...
        laserStateONIconLabel.place(relx=0.0335, rely = 0.4025, anchor='nw') #places the on laser image
    else:
        laserStateONIconLabel.place_forget() #hides the on laser image
        laserStateOFFIconLabel.place(relx=0.0335, rely = 0.4025, anchor='nw') #places the off laser image
    try:
...
class SimulationApparatus(SimulationObject): #defines a class for the screen and grating instances within the simulation environment
    ...
    def setSprite(self, newSprite):
        ...
        selfImage = PhotoImage(file=get_asset_path(self.sprite))
...
class MeasuringEquipment(SimulationObject): #defines a class for the ruler and protractor instances within the simulation environment
    def __init__(self, initialXCord, initialYCord, initialSprite, initialOrientation, initialZoom, initialRotateLabelOffset):
        ...
        self.rotateIcon = PhotoImage(file=get_asset_path("RotateIcon.png"))
    ...
    def setSprite(self, newSprite):
        self.sprite = newSprite
        self.selfImage = PhotoImage(file=get_asset_path(self.sprite))
        if self.zoom != 1:
    ...
    def rotate(self, event): #method to rotate the measurement object by 45 degrees
        ...
        if self.orientation == 315:
            self.orientation = 0
            selfImage = PhotoImage(file=get_asset_path((self.spriteType + "Icon.png")))
            ...
            self.orientation += 45
            selfImage = PhotoImage(file=get_asset_path((self.spriteType + "IconRotated" + str(self.orientation) + "Degrees.png")))
            selfImage = selfImage.subsample(self.zoom)
