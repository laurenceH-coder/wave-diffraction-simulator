...
def generateSystem(systemValues):
    ...
    #ruler = MeasuringEquipment(0.085, 0.9, "rulerIcon.png", 0, 4) #creates the ruler object
    #protractor = MeasuringEquipment(0.085, 0.79, "protractorIcon.png", 0, 3) #creates the protractor object
    ...
    rulerIcon = PhotoImage(file = "rulerIcon.png")
    rulerIcon = rulerIcon.subsample(4)
    ruler = Label(rayLineCanvas,image = rulerIcon, border = 0)
    ruler.image = rulerIcon
    ruler.place(x=100,y=100, anchor = "center")

    protractorIcon = PhotoImage(file = "protractorIcon.png")
    protractorIcon = protractorIcon.subsample(3)
    protractor = Label(rayLineCanvas,image = protractorIcon, border = 0)
    protractor.image = protractorIcon
    protractor.place(x=100,y=100, anchor = "center")


    ruler.bind("<Button-1>",drag_start)
    ruler.bind("<B1-Motion>",drag_motion)

    protractor.bind("<Button-1>",drag_start)
    protractor.bind("<B1-Motion>",drag_motion)
...
class MeasuringEquipment(SimulationObject): #defines a class for the ruler and protractor instances within the simulation environment
    def __init__(self, initialXCord, initialYCord, initialSprite, initialOrientation, initialZoom):
        super().__init__(initialXCord, initialYCord)
        self.startX = 0
        self.startY = 0
    ...
    def setSprite(self, newSprite):
        ...
        self.objectLabel = Label(rayLineCanvas, image = self.selfImage)
        self.objectLabel.config(image=self.selfImage)
        self.objectLabel.image = self.selfImage
        self.objectLabel.place_forget()
        self.objectLabel.place(x = int(windowWidth)*self.xCord, y = int(windowHeight)*self.yCord, anchor='center') #updates the sprite of the object
        print(self.objectLabel.winfo_x())
        print(self.objectLabel.winfo_y())
    def getSprite(self):
        return self.sprite #get method to access the sprite of the object
    def display(self):
        self.objectLabel.place(relx = self.xCord, rely = self.yCord, anchor = 'center') #displays object's label at updated position
    def hide(self):
        self.objectLabel.place_forget() #hides the object's label from the user's view
    ...
    def on_drag(self,event):
        ...
        print(self.root.winfo_pointerx()-self.root.winfo_rootx(), self.root.winfo_pointery()-self.root.winfo_rooty())
