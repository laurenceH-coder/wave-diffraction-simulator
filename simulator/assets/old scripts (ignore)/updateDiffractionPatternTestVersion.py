...
def generateSystem(systemValues):
    ...
    ruler = MeasuringEquipment(0.5, 0.5, "rulerIcon.png", 0, "RULER", 4)
...
class MeasuringEquipment(SimulationObject): #defines a class for the ruler and protractor instances within the simulation environment
    def __init__(self, initialXCord, initialYCord, initialSprite, initialOrientation, initialTag, initialZoom):
        rayLineCanvas.grid(row=0,column=0)
        self._cLabel = Label(rayLineCanvas, text='Hello World', bg='blue')
        self.id_cLabel = rayLineCanvas.create_window(500,500, window=self._cLabel,    tags="motion_bound")
        #self.update()
        self._cLabel.bind("<B1-Motion>", self.relocate("buttonMotion"))
        ...
        super().__init__(initialXCord, initialYCord)
        self.sprite = initialSprite #constructor method initialises the variables of the simulation apparatus object and displays its label with the parent constructor
        self.setOrientation(initialOrientation)
        self.display()

    def relocate(self, event):
        print(rayLineCanvas.winfo_pointerxy())
        x0,y0 = rayLineCanvas.winfo_pointerxy()
        x0 -= rayLineCanvas.winfo_rootx()
        y0 -= rayLineCanvas.winfo_rooty()
        rayLineCanvas.coords(self.id_cLabel,x0,y0)
        
    def setSprite(self, newSprite):
        self.sprite = newSprite
        self.selfImage = PhotoImage(file=self.sprite)
        #self.selfImage = self.selfImage.subsample(self.zoom)
        tempImage= self.selfImage
        rayLineCanvas.tempImage = tempImage
        self.objectImage = rayLineCanvas.create_image(self.xCord,self.yCord,image=self.selfImage,anchor='center', tag = self.tag)
    def getSprite(self):
        return self.sprite #get method to access the sprite of the object
    def setTag(self, newTag): #updates the tag of the object
        self.tag = newTag
    def getTag():
        return self.tag #get method to access the tag of the object
