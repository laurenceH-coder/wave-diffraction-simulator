...
def generateSystem(systemValues):
    ...
    slitSeparation.set(1.500) #initialises the slitSeparation value as 1.500
...
def updateDiffractionPattern():
    ...
    if gratingState.get() == True: #checks that the grating is enabled before calculating and plotting the diffraction pattern
        ...
        heightBetweenMaximas = heightBetweenMaximas*0.13*int(windowHeight)
            ...
            if chosenDiffractionType.get() == "Single-Slit" or chosenDiffractionType.get() == "Double-Slit":
                for y in range(0, countMaximas+1):
                    currentRayLine = rayLineCanvas.create_line(slitPositions[x][0]*(int(windowWidth))+7,
                                              ...
                                              width=3,
                                              fill=rayLineColour) #draws a rayline from the current slit to the maxima at distance y * heightBetweenMaximas vertically
                                                                  #below the centre of the screen where y is the counter variable for the number of times this inner for
                                                                  #loop must iterate to plot raylines to all the maximas
                    currentRayLine = rayLineCanvas.create_line(slitPositions[x][0]*(int(windowWidth))+7,
                                              slitPositions[x][1]*(int(windowHeight)),
                                              screenPosition[0]*(int(windowWidth)),
                                              screenPosition[1]*(int(windowHeight))-(y*heightBetweenMaximas),
                                              width=3,
                                              fill=rayLineColour) #draws a rayline from the current slit to the maxima at distance y * heightBetweenMaximas vertically
                                                                  #above the centre of the screen
            #Diffraction grating:
            else:
                print(x)
                slitToMaximasDictionary = {}
                slitToMaximasDictionary[1] = [str(countMaximas) + "u", str(countMaximas-1) + "u"]
                for z in range(-countMaximas, countMaximas):
                    print("1:" + str((screenPosition[1])*(int(windowHeight))-(z*heightBetweenMaximas)))
                    print("2:" + str(slitPositions[x][1]*(int(windowHeight))))
                    print("3:" + str(heightBetweenMaximas))
                    if (screenPosition[1])*(int(windowHeight))-(z*heightBetweenMaximas) < slitPositions[x][1]*(int(windowHeight)):
                        slitHeightToUpperDistance = slitPositions[x][1]*(int(windowHeight)) - ((screenPosition[1])*(int(windowHeight))-(z*heightBetweenMaximas))
                        slitHeightToLowerDistance = (screenPosition[1])*(int(windowHeight))-((z-1)*heightBetweenMaximas) - slitPositions[x][1]*(int(windowHeight))
                        print("slitHeightToUpperDistance: " + str(slitHeightToUpperDistance)) #correct value
                        print("slitHeightToLowerDistance: " + str(slitHeightToLowerDistance)) #correct value
                        if slitHeightToUpperDistance >= slitHeightToLowerDistance: #if slit is closer to lower maxima
                            positionMiddle = (slitPositions[x][1]*(int(windowHeight))+slitHeightToUpperDistance)
                            positionUpper = (positionMiddle - heightBetweenMaximas)
                            positionLower = (positionMiddle + heightBetweenMaximas)
                            print("Position upper: " + str(positionUpper))
                            print("Position middle: " + str(positionMiddle))
                            print("Position lower: " + str(positionLower))

                            
                        else: #if slit closer to upper
                            positionMiddle = (slitPositions[x][1]*(int(windowHeight))-slitHeightToUpperDistance)
                            positionUpper = (positionMiddle - heightBetweenMaximas)
                            positionLower = (positionMiddle + heightBetweenMaximas)
                            print("Position upper: " + str(positionUpper))
                            print("Position middle: " + str(positionMiddle))
                            print("Position lower: " + str(positionLower))
                        print("Slit position:" + str(screenPosition[1]*(int(windowHeight))))
                            
                        #Plotting lines
                        currentRayLine = rayLineCanvas.create_line(slitPositions[z][0]*(int(windowWidth))+7,
                                                  slitPositions[x][1]*(int(windowHeight)),
                                                  screenPosition[0]*(int(windowWidth))-10,
                                                  positionUpper,
                                                  width=3,
                                                  fill=rayLineColour) #draws a rayline from the current slit to the maxima at distance y * heightBetweenMaximas vertically
                                                                      #above the centre of the screen
                        ...
                        currentRayLine = rayLineCanvas.create_line(slitPositions[z][0]*(int(windowWidth))+7,
                                                  ...
                                                  fill=rayLineColour) #draws a rayline from the current slit to the maxima at distance y * heightBetweenMaximas vertically
                                                                      #above the centre of the screen
                        break
        print(heightBetweenMaximas)
                            
                        
                #for z in range(2, numberOfSlits//2):
                    #slitPositions[z][1]*(int(windowHeight))
                    #slitToMaximasDictionary[z] = [str(countMaximas) + "u", str(countMaximas-1) + "u"]
                #slitToMaximasDictionary[numberOfSlits] = [str(countMaximas) + "d", str(countMaximas-1) + "d"]
